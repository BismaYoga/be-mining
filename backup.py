...
# Import InMemorySessionService untuk menyimpan riwayat chat di memori
...
# --- KONFIGURASI AWAL DAN PEMUATAN ENV ---
...
# --- PEMUATAN MODEL PREDIKSI DARI .pkl (Global) ---
...
# --- INISIALISASI AGENT (Global) ---
AGENT_NAME = "mining_prediction_agent"
# APP_NAME diatur ke 'agents' untuk menghindari kesalahan mismatch agent ADK
APP_NAME = "agents" 
GEMINI_MODEL = "gemini-2.5-flash" 
# Session Service yang menyimpan sesi di memori
session_service = InMemorySessionService() 
mining_agent: Optional[LlmAgent] = None

# --------------------------------------------------------------------------------------------------
# --- DEFINISI TOOL (FUNGSI PREDIKSI) ---
...
def predict_mining_target(scenarios: List[List[Union[int, float]]]) -> str:
    ...
    try:
        ...
        # Mengembalikan string terstruktur agar Agent dapat menggunakannya
...
# --- FUNGSI INTI UNTUK MENGINISIALISASI AGENT ---
def initialize_agent():
    ...
    **Tugas Utama (Untuk Setiap Respon):**
    ...
    4.  **Parsing dan Struktur Output (WAJIB KETAT):**
        ...
        b. **OUTPUT PART 1 (Analisis Awal):** Sajikan analisis Skenario Kontrol.
...
class MiningInput(BaseModel):
    ...
    query: str = Field(..., example="Saya ingin 12500 ton. Saat ini pakai 12 Truk, 3 Ekskavator, 18 Operator, cuaca Cloudy. Beri 3 rekomendasi!", description="Pesan teks bebas yang berisi Target dan parameter tambang.")
...
def parse_agent_response(text: str) -> Dict[str, Union[str, float, int, List[Dict]]]:
    ...
    # 2. Ekstrak Detail Analisis Awal (Target, Prediksi Kontrol, Selisih Kontrol)
    ...
    for rec_block in recs_list:
        ...
        # Ekstrak semua parameter termasuk Alasan (Rationale)
            ...
            if python_key:
                ...
                try:
                    if python_key in ['trucks', 'excavators', 'operators']:
                        # Menggunakan int(float(...)) untuk menangani input numerik dengan desimal
    ...
    # Validasi output untuk mencegah NULL pada Pydantic
...
# --------------------------------------------------------------------------------------------------
# --- ENDPOINT FASTAPI (Sesi Berlanjut - Disesuaikan) ---
...
async def predict_and_recommend(data: MiningInput):
    ...
    # Logika untuk memastikan sesi ada atau dibuat (Upsert). 
    # Karena kita menggunakan InMemorySessionService, kita coba buat sesi. 
    # Jika sesi sudah ada, ADK akan melempar error, yang kita tangkap dan abaikan (pass).
    try:
        await session_service.create_session(app_name=APP_NAME, user_id=user_id, session_id=session_id)
        # Tidak perlu ada `asyncio.sleep`, Runner akan beroperasi dengan sesi yang baru dibuat.
    except Exception:
        # Sesi sudah ada, biarkan Runner mengambil riwayat obrolan dari sesi yang sudah ada.
        pass
        
    # Runner dibuat dengan agent dan session_service yang sama.
    runner = Runner(agent=mining_agent, app_name=APP_NAME, session_service=session_service)

    # Menjalankan Agent
    final_response_text = ""
    try:
        # Gunakan user_id dan session_id yang sama untuk menjaga konteks chat
...
# --------------------------------------------------------------------------------------------------
# Endpoint tambahan untuk menghapus sesi (opsional, untuk menguji sesi baru)
